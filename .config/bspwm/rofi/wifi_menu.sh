...
ssid="$(grep -v "${current_wifi}" "${NETWORKS_CACHE}" | rofi -dmenu -p "Networks" -theme "${ROFI}/wifi_menu.rasi" | awk '{print $1}')"

if [ ! -z "$ssid" ]; then
    password=$(rofi -i -dmenu -password -no-fixed-num-lines -p "Password: " -theme ${ROFI}/input.rasi)
    if [ -z "$password" ]; then
